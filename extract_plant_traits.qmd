...
check_if_join <- function(dataset_id_to_check, entity_context_id_to_check, contexts) {
  (contexts %>% filter(dataset_id_to_check == dataset_id, str_detect(entity_context_id, entity_context_id_to_check)) %>% nrow()) > 0
}

recovery_traits <-
  recovery_traits %>%
  rowwise() %>%
  mutate(to_join = check_if_join(dataset_id, entity_context_id, contexts))
...
new_recovery_traits <- recovery_traits %>%
  left_join(contexts, by = c("dataset_id", "to_join"))

new_recovery_traits <-
  new_recovery_traits %>%
  filter(str_detect(entity_context_id.y, entity_context_id.x) | is.na(entity_context_id.y))
```
...
austraits$traits %>%
  filter(trait_name %in% c("resprouting_capacity", "post_fire_recruitment")) %>%
  group_by(taxon_name) %>%
  summarise(
    resprouts = sum(str_detect(value, "resprout")) > 0,
    post_fire_seeder = sum(str_detect(value, "(?<!_)post_fire_recruitment")) > 0) %>%
